...
def constructBayesNet(gameState: hunters.GameState):
    ...
    netEdges = []
    domainOfVariables = {}
    netVariables.extend([PAC, GHOST0, GHOST1, OBS0, OBS1])
    netEdges.append((GHOST0, OBS0))
    netEdges.append((PAC, OBS0))
    netEdges.append((PAC, OBS1))
    netEdges.append((GHOST1, OBS1))
    ...
    domainOfVariables[PAC] = allPossiblePositions
    domainOfVariables[GHOST0] = allPossiblePositions
    domainOfVariables[GHOST1] = allPossiblePositions

    maxManhattanDistance = (X_RANGE - 1) + (Y_RANGE - 1)
    upperLimit = maxManhattanDistance + MAX_NOISE + 1
    possibleValues = range(upperLimit)
    domainOfVariables[OBS0] = list(possibleValues)
    domainOfVariables[OBS1] = list(possibleValues)
    net = bn.constructEmptyBayesNet(netVariables, netEdges, domainOfVariables)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factorsContainingVariable = []
            for factor in factors:
                if variable in factor.variablesSet():
                    factorsContainingVariable.append(factor)

            if factorsContainingVariable:
                _, joinedFactor = joinFactorsByVariable(factors, variable)

                if len(joinedFactor.unconditionedVariables()) > 1:
                    newFactor = eliminate(joinedFactor, variable)
                    updatedFactors = [newFactor]
                    for factor in factors:
                        if variable not in factor.variablesSet():
                            updatedFactors.append(factor)
                else:
                    updatedFactors = []
                    for factor in factors:
                        if variable not in factor.variablesSet():
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in allPositions:
            ...
            probabilityObservation = self.getObservationProb(observation, pacmanCurrentPosition, position, jailPosition)
            newBeliefs[position] = priorBelief * probabilityObservation
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for currentPosition in self.allPositions:
            newPositionProbability = self.getPositionDistribution(gameState, currentPosition)
            for newPositionProbability, transitionProbability in newPositionProbability.items():
                updatedBeliefs[newPositionProbability] += transitionProbability * self.beliefs[currentPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """
        self.particles = []
        allPositions= len(self.legalPositions)
        for particle in range(self.numParticles):
            particlePosition = self.legalPositions[particle % allPositions]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for currentParticle in self.particles:
            weightDistribution[currentParticle] += self.getObservationProb(observation, currentPosition, currentParticle, jailPosition)

        if weightDistribution.total() != 0:
            updatedParticles = []
            for _ in range(self.numParticles):
                updatedParticles.append(weightDistribution.sample())
    ...
    def elapseTime(self, gameState):
        ...
        """
        updatedParticles = []
        for currentPosition in self.particles:
            newPositionDistance = self.getPositionDistribution(gameState, currentPosition)
            updatedParticle = newPositionDistance.sample()
